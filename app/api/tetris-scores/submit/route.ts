...
import { parseTetrisScoreInput } from "@/lib/games/tetris-score-validation";
...
export async function POST(request: Request) {
  ...
  const parsed = parseTetrisScoreInput(body);
  ...
  if (!session || session.submitted_at || session.game_key !== "tetris" || session.difficulty !== "classic") {
    return NextResponse.json({ error: "이미 제출됐거나 유효하지 않은 게임 세션입니다." }, { status: 409 });
  }

  const now = new Date();
  const serverDurationMs = now.getTime() - new Date(session.started_at).getTime();
  if (serverDurationMs < 0 || serverDurationMs > 3 * 60 * 60 * 1000 ||
      Math.abs(serverDurationMs - score.durationMs) > Math.max(MAX_DURATION_DRIFT_MS, serverDurationMs * 0.25)) {
    return NextResponse.json({ error: "플레이 시간과 제출 기록이 일치하지 않습니다." }, { status: 400 });
  }

  const submissionHash = createHash("sha256")
    .update(`${user.id}:tetris:${session.id}:${score.score}:${score.lines}:${score.level}`)
    .digest("hex");

  const { data: submitResult, error: submitError } = await admin.rpc("makeon_submit_tetris_score", {
    p_user_id: user.id,
    ...
    p_difficulty: "classic",
    p_submission_hash: submissionHash,
    p_score_date: kstDate(now),
  });
  if (submitError) {
  ...
  try {
    const rankings = await loadTetrisRankings(admin, user.id, "all");
    return NextResponse.json({
      saved: improved,
      improved,
      ignored: !improved,
