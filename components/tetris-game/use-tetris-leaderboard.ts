...
export function useTetrisLeaderboard() {
  ...
  const refreshRankings = useCallback(async (nextPeriod: RankingPeriod) => {
    ...
    try {
      const response = await authorizedFetch(`/api/tetris-scores/rankings?type=${nextPeriod}`);
      if (!response.ok) throw new Error(await responseError(response));
  ...
  const createGameSession = useCallback(async () => {
    ...
    try {
      const response = await authorizedFetch("/api/tetris-scores/session", {
        method: "POST",
        body: JSON.stringify({ gameKey: "tetris", difficulty: "classic" }),
      });
      if (!response.ok) throw new Error(await responseError(response));
  ...
  const submitTetrisScore = useCallback(async (result: { score: number; lines: number; level: number }) => {
    const session = activeSessionRef.current;
    activeSessionRef.current = null;
    if (!configured || !session || result.score <= 0) return;

    setSubmissionStatus("submitting");
    setSubmissionError(null);
    try {
      const response = await authorizedFetch("/api/tetris-scores/submit", {
        method: "POST",
        body: JSON.stringify({
          gameKey: "tetris",
          ...result,
      ...
      setSubmissionError(error instanceof Error ? error.message : "기록을 저장하지 못했습니다. 브라우저 최고 기록은 유지됩니다.");
